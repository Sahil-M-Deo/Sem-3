...
def build(n):
    ...
    # everyone starts on the left...
    for e in ENTITIES:
        s.add(L[e][0])
    # ...and everyone must end on the right
    for e in ENTITIES:
        s.add(Not(L[e][n]))

    for t in range(n):
        # (b) the boat: the farmer always crosses, and takes at most one
        #     passenger, who must already be standing on his bank.
        s.add(L[FARMER][t + 1] == Not(L[FARMER][t]))

        s.add(AtMost(*[M[e][t] for e in CARGO], 1))

        for e in CARGO:
            # a passenger boards only from the farmer's current bank
            s.add(Implies(M[e][t], L[e][t] == L[FARMER][t]))
            # ...and if it boards, it ends up across the river
            s.add(Implies(M[e][t], L[e][t + 1] == Not(L[e][t])))

            # (c) the frame axiom: nothing moves unless it was moved.
            #     This is the minimal statement -- it says nothing about
            #     *where* e is, only that its position is unchanged.
            s.add(Implies(Not(M[e][t]), L[e][t + 1] == L[e][t]))

    # (d) safety -- at *every* time, not just across transitions
    for t in range(n + 1):
        for a, b in CONFLICTS:
            together = L[a][t] == L[b][t]          # a and b share a bank
            unwatched = L[FARMER][t] != L[a][t]    # farmer is elsewhere
            s.add(Not(And(together, unwatched)))
